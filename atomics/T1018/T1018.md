...
- [Atomic Test #4 - Remote System Discovery - ping sweep - Windows](#atomic-test-4---remote-system-discovery---ping-sweep---windows)
...
- [Atomic Test #7 - Remote System Discovery - ping sweep - Linux](#atomic-test-7---remote-system-discovery---ping-sweep---linux)
...
- [Atomic Test #22 - File Read - /etc/hosts](#atomic-test-22---file-read---etchosts)
...
## Atomic Test #7 - Remote System Discovery - ping sweep - Linux
Identify remote systems via ping sweep.
...
| subnet | Subnet used for ping sweep. | string | 192.168.1|
| timeout | Timeout for commands in seconds | int | 10|
...
```sh
(timeout #{timeout} bash -c 'for ip in $(seq #{start_host} #{stop_host}); do ping -c 1  #{subnet}.$ip; [ $? -eq 0 ] && echo "#{subnet}.$ip UP" || : ; done') || :
```
...
## Atomic Test #22 - File Read - /etc/hosts
Detects file read events on /etc/hosts

**Supported Platforms:** Linux


**auto_generated_guid:** 1e4f5b8c-b22a-48e8-8b9e-0e3fb7ab82c3
