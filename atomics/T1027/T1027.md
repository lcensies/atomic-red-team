...
- [Atomic Test #2 - Execute Python Code via eval with Base64 Decoding](#atomic-test-2---execute-python-code-via-eval-with-base64-decoding)
...
- [Atomic Test #9 - Obfuscated Command Line using special Unicode characters](#atomic-test-9---obfuscated-command-line-using-special-unicode-characters)
...
## Atomic Test #2 - Execute Python Code via eval with Base64 Decoding
Executes arbitrary Python code by simulating the use of `eval` to execute base64 decoded data. This technique can be used for obfuscation and to bypass simplistic detection mechanisms.

**Supported Platforms:** Linux


**auto_generated_guid:** 47be8c8d-0cbb-4fcf-86c9-80fabd3d712d
...
```bash
# Example Python command using eval with base64 decoding of a simple print statement
PYTHON_CMD=$(which python || which python3)
$PYTHON_CMD -c "eval(base64.b64decode('cHJpbnQoIkhlbGxvLCB3b3JsZCIp'))"
$PYTHON_CMD -c "eval(b64decode('cHJpbnQoIkV2YWwgV2l0aCBCYXNlNjQgRGVjb2RpbmciKQ=='.encode('ascii')))"
```
...
##### Description: Requires Python to be installed on the system.
##### Check Prereq Commands:
```bash
...
##### Get Prereq Commands:
```bash
echo "Python is not installed. Install Python to proceed." && false
```
...
## Atomic Test #10 - Snake Malware Encrypted crmlog file
